/**
 * DSE 修練所 - 後端核心程式 (V66: 讀寫分離 + LockService 防連點 + 終極分數查核)
 * 運行於 Google 美國資料中心
 */

// 🚨 請在這裡直接貼上你的 API 金鑰！
const GEMINI_API_KEY = "請在這裡直接貼上你的API金鑰"; 
const BACKEND_VERSION = "V66";
...
function doPost(e) {
  try {
    ...
    // =====================================
    // 🛡️ 啟動排隊鎖 (LockService) 專注保護資料庫寫入
    // =====================================
    const lock = LockService.getScriptLock();
    try {
      lock.waitLock(8000); // 最多排隊等待 8 秒
    } catch (e) {
    ...
    try {
      ...
      // 🛡️ 閘門 3：分數合理性查核 (Modulo Check)
      let isValidScore = false;
      if (isHomework) {
          // 功課：強制所有題目為 10 分，總分必須是 10 的倍數
          if (score % 10 === 0) isValidScore = true;
      } else {
          // 日常練習依據難度配分檢查
          if (levelStr.includes('綜合挑戰')) {
             isValidScore = true; // 綜合難度分數混合，不檢查模數
      ...
      if (isHomework) {
          ...
          // 迴圈掃描：比對 班別(C欄, index 2)、學號(D欄, index 3)、功課名稱(F欄, index 5)
          for (let i = 1; i < hwRecordData.length; i++) {
              if (String(hwRecordData[i][2]).toUpperCase() === cleanClassName && 
                  String(hwRecordData[i][3]) === cleanClassNum && 
                  String(hwRecordData[i][5]) === homeworkName) {
          ...
          hwRecordSheet.appendRow([todayStr, formattedTime, cleanClassName, cleanClassNum, officialStudentName, homeworkName, score, totalScore, p.percentage || "", topicDetails, "功課紀錄"]);
          
          // 🌟 功課獨立：不計入日常積分，避免干擾排行榜
          return jsonResponse({ 
              success: true, 
              message: `✅ 功課傳送成功！(第 ${submitCountForHomework + 1}/2 次)\n(註：專屬功課分數獨立計算，不計入日常排行榜)`, 
              playCountToday: playCountToday, 
              newTotalScore: currentTotalScore, // 維持原積分不變
              crossedThreshold: false, 
              reward: "無", 
              officialName: officialStudentName 
          });
