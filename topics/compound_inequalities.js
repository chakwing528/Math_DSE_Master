...
function _ciBuildEq(steps) {
    return steps.map(s => `<div class="my-1">${s.text}</div>`).join('');
}

function _ciWrapHint(msg, stepsHtml) {
    return `
    ...
    for (let i = 0; i < num; i++) {
        ...
        if (levelType === '1') {
            ...
            if (subType === 0) {
                ...
                options = [
                    { text: `\\( x > ${ansStr} \\)`, isCorrect: true, hint: _ciWrapHint(`<div class="text-green-600 font-bold text-lg">解題正確！</div>`, _ciBuildEq(steps)) },
                    { text: `\\( x < ${ansStr} \\)`, isCorrect: false, hint: _ciWrapHint(msgIneqSign, _ciBuildEq(steps)) },
                    { text: `\\( x > \\frac{${a+c}}{${b}} \\)`, isCorrect: false, hint: _ciWrapHint(`<div class="text-red-600 font-bold text-lg">❗ 移項正負號錯誤</div>`, _ciBuildEq(steps)) },
                    { text: `\\( x < \\frac{${a+c}}{${b}} \\)`, isCorrect: false, hint: _ciWrapHint(msgIneqSign, _ciBuildEq(steps)) }
                ];
            } else if (subType === 1) {
                ...
                options = [
                    { text: "只有 II 及 III", isCorrect: true, hint: _ciWrapHint(`<div class="text-green-600 font-bold text-lg">解題正確！</div>`, _ciBuildEq(steps)) },
                    { text: "只有 I 及 II", isCorrect: false, hint: _ciWrapHint(msgProp + "<div class='text-sm text-slate-500'>提示：考慮 a 為正數而 b 為負數的極端情況（如 1 > -2）。</div>", _ciBuildEq(steps)) },
                    { text: "只有 I 及 III", isCorrect: false, hint: _ciWrapHint(msgProp, _ciBuildEq(steps)) },
                    { text: "I、II 及 III", isCorrect: false, hint: _ciWrapHint(msgProp, _ciBuildEq(steps)) }
                ];
            } else {
                ...
                options = [
                    { text: "\\( -2 < x < 3 \\)", isCorrect: true, hint: _ciWrapHint(`<div class="text-green-600 font-bold text-lg">解題正確！</div>`, _ciBuildEq(steps)) },
                    { text: "\\( x > -2 \\)", isCorrect: false, hint: _ciWrapHint(msgAndOr + "<div class='text-sm text-slate-500'>提示：這只是其中一邊的解，必須與另一邊取共同範圍。</div>", _ciBuildEq(steps)) },
                    { text: "\\( x < 3 \\)", isCorrect: false, hint: _ciWrapHint(msgAndOr, _ciBuildEq(steps)) },
                    { text: "無解", isCorrect: false, hint: _ciWrapHint(msgAndOr, _ciBuildEq(steps)) }
                ];
            }
            ...
            if (subType === 0) {
                ...
                options = [
                    { text: "\\( x < -7 \\)", isCorrect: true, hint: _ciWrapHint(`<div class="text-green-600 font-bold text-lg">解題正確！</div>`, _ciBuildEq(steps)) },
                    { text: "\\( x < 3 \\)", isCorrect: false, hint: _ciWrapHint(msgAndOr + "<div class='text-sm text-slate-500'>提示：x < 3 包含了不屬於 x < -7 的區域，並非交疊共同範圍。</div>", _ciBuildEq(steps)) },
                    { text: "\\( -7 < x < 3 \\)", isCorrect: false, hint: _ciWrapHint(msgAndOr, _ciBuildEq(steps)) },
                    { text: "\\( x < -7 \\) 或 \\( x > 3 \\)", isCorrect: false, hint: _ciWrapHint(msgAndOr, _ciBuildEq(steps)) }
                ];
            } else if (subType === 1) {
                ...
                options = [
                    { text: "\\( x < 4 \\)", isCorrect: true, hint: _ciWrapHint(`<div class="text-green-600 font-bold text-lg">解題正確！</div>`, _ciBuildEq(steps)) },
                    { text: "\\( x < -3 \\)", isCorrect: false, hint: _ciWrapHint(msgAndOr + "<div class='text-sm text-slate-500'>提示：這是「或 (OR)」，需要取聯集(合併)，不是取交集。</div>", _ciBuildEq(steps)) },
                    { text: "\\( -3 < x < 4 \\)", isCorrect: false, hint: _ciWrapHint(msgAndOr, _ciBuildEq(steps)) },
                    { text: "全體實數", isCorrect: false, hint: _ciWrapHint(msgAndOr, _ciBuildEq(steps)) }
                ];
            } else {
                ...
                options = [
                    { text: "無解", isCorrect: true, hint: _ciWrapHint(`<div class="text-green-600 font-bold text-lg">解題正確！</div>`, _ciBuildEq(steps)) },
                    { text: `\\( -${k} < x < 2 \\)`, isCorrect: false, hint: _ciWrapHint(msgAndOr, _ciBuildEq(steps)) },
                    { text: `\\( x < -${k} \\) 或 \\( x > 2 \\)`, isCorrect: false, hint: _ciWrapHint(msgAndOr, _ciBuildEq(steps)) },
                    { text: "全體實數", isCorrect: false, hint: _ciWrapHint(msgAndOr, _ciBuildEq(steps)) }
                ];
            }
            ...
            if (subType === 0) {
                ...
                options = [
                    { text: "\\( x > 9 \\)", isCorrect: true, hint: _ciWrapHint(`<div class="text-green-600 font-bold text-lg">解題正確！</div>`, _ciBuildEq(steps)) },
                    { text: "\\( x > 12 \\)", isCorrect: false, hint: _ciWrapHint(msgAndOr + "<div class='text-sm text-slate-500'>提示：這是「或 (OR)」，大於 9 的數（如 10）也是合法的解，不應被排除。</div>", _ciBuildEq(steps)) },
                    { text: "\\( 9 < x < 12 \\)", isCorrect: false, hint: _ciWrapHint(msgAndOr, _ciBuildEq(steps)) },
                    { text: "\\( x > 11 \\)", isCorrect: false, hint: _ciWrapHint(`<div class="text-red-600 font-bold text-lg">❗ 分式展開負號錯誤</div>`, _ciBuildEq(steps)) }
                ];
            } else if (subType === 1) {
                // Type 1: 雙邊分數交叉相乘
                ...
                options = [
                    { text: "\\( x \\le \\frac{17}{7} \\)", isCorrect: true, hint: _ciWrapHint(`<div class="text-green-600 font-bold text-lg">解題正確！</div>`, _ciBuildEq(steps)) },
                    { text: "\\( x \\ge \\frac{17}{7} \\)", isCorrect: false, hint: _ciWrapHint(msgIneqSign, _ciBuildEq(steps)) },
                    { text: "\\( x \\le \\frac{7}{7} \\)", isCorrect: false, hint: _ciWrapHint(`<div class="text-red-600 font-bold text-lg">❗ 括號展開常數項未乘係數</div>`, _ciBuildEq(steps)) },
                    { text: "\\( x \\ge 1 \\)", isCorrect: false, hint: _ciWrapHint(msgIneqSign, _ciBuildEq(steps)) }
                ];
            } else {
                ...
                options = [
                    { text: "\\( x \\le -2 \\)", isCorrect: true, hint: _ciWrapHint(`<div class="text-green-600 font-bold text-lg">解題正確！</div>`, _ciBuildEq(steps)) },
                    { text: "\\( x < 6 \\)", isCorrect: false, hint: _ciWrapHint(msgAndOr, _ciBuildEq(steps)) },
                    { text: "\\( -2 \\le x < 6 \\)", isCorrect: false, hint: _ciWrapHint(msgAndOr, _ciBuildEq(steps)) },
                    { text: "\\( x \\ge -2 \\)", isCorrect: false, hint: _ciWrapHint(msgIneqSign, _ciBuildEq(steps)) }
                ];
            }
        } else {
            ...
            if (subType === 0) {
                ...
                options = [
                    { text: `\\( x < ${r1} \\) 或 \\( x > ${r2} \\)`, isCorrect: true, hint: _ciWrapHint(`<div class="text-green-600 font-bold text-lg">解題正確！</div>`, _ciBuildEq(steps)) },
                    { text: `\\( ${r1} < x < ${r2} \\)`, isCorrect: false, hint: _ciWrapHint(msgQuad, _ciBuildEq(steps)) },
                    { text: `\\( x > ${r2} \\)`, isCorrect: false, hint: _ciWrapHint(msgQuad, _ciBuildEq(steps)) },
                    { text: `\\( x < ${r1} \\)`, isCorrect: false, hint: _ciWrapHint(msgQuad, _ciBuildEq(steps)) }
                ];
            } else if (subType === 1) {
                // Type 1: 不等式的整數解計數文字題
                ...
                options = [
                    { text: "\\( 15 < k \\le 18 \\)", isCorrect: true, hint: _ciWrapHint(`<div class="text-green-600 font-bold text-lg">解題正確！</div>`, _ciBuildEq(steps)) },
                    { text: "\\( 15 \\le k < 18 \\)", isCorrect: false, hint: _ciWrapHint(`<div class="text-red-600 font-bold text-lg">❗ 邊界等號判斷錯誤</div>`, _ciBuildEq(steps)) },
                    { text: "\\( 9 < k \\le 12 \\)", isCorrect: false, hint: _ciWrapHint(`<div class="text-red-600 font-bold text-lg">❗ 整數解選取錯誤</div>`, _ciBuildEq(steps)) },
                    { text: "\\( 15 < k < 18 \\)", isCorrect: false, hint: _ciWrapHint(`<div class="text-red-600 font-bold text-lg">❗ 漏掉邊界等號</div>`, _ciBuildEq(steps)) }
                ];
            } else {
                ...
                options = [
                    { text: "\\( k > 11 \\)", isCorrect: true, hint: _ciWrapHint(`<div class="text-green-600 font-bold text-lg">解題正確！</div>`, _ciBuildEq(steps)) },
                    { text: "\\( k < 11 \\)", isCorrect: false, hint: _ciWrapHint(msgIneqSign, _ciBuildEq(steps)) },
                    { text: "\\( k > -7 \\)", isCorrect: false, hint: _ciWrapHint(`<div class="text-red-600 font-bold text-lg">❗ 常數項正負號移項錯誤</div>`, _ciBuildEq(steps)) },
                    { text: "\\( k < -7 \\)", isCorrect: false, hint: _ciWrapHint(msgIneqSign, _ciBuildEq(steps)) }
                ];
            }
        }
        ...
        while(options.length < 4) {
            options.push({ text: `\\( x > ${options.length * 5} \\)`, isCorrect: false, hint: _ciWrapHint(msgIneqSign, _ciBuildEq(steps)) });
        }
        qObj.options = _ciShuffle(options).map((opt, idx) => ({
            ...opt,
